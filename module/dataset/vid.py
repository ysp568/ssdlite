...
class VIDDataset:
	#def __init__(self, data, , preproc=None, target_transform=AnnotationTransform()):
	def __init__(self, data, root, preproc=None, is_val=False,batch_size=None):
	#def __init__(self, data, root, transform=None, target_transform=None, is_val=False, batch_size=None, label_file=None):
		...
		if self.is_val:
			...
			image_sets_file = "module/dataset/train_VID_seqs_list.txt"
		self.seq_list = VIDDataset._read_image_seq_ids(image_sets_file)
		rem = len(self.seq_list)%self.batch_size
		
		self.seq_list = self.seq_list[:-(rem)]  #这是去掉那些不能组成一个bach的数据
		...
		# 	image_sets_file = "datasets/val_VID_seqs_list.txt"
		# else:
		# 	image_sets_file = "module/datasets/train_VID_seqs_list.txt"
		# self.seq_list = VIDDataset._read_image_seq_ids(image_sets_file)
	...
	def __getitem__(self, index):
		...
		targets_seq = data['targets_seq']
		images = []
		new_targets_seq=[]
		#start = random.randint(0,len(images_seq)-10)
		for i in range(0,len(images_seq)):
		#for i in range(start,start+10):
			image = self._read_image(images_seq[i])
			height, width,_ = image.shape
			targets = targets_seq[i]
			...
			images.append(image)
			new_targets_seq.append(targets)
	...
	def gt_roidb(self):
		...
		#gt_roidb = [self.load_vid_annotation(index) for index in range(0, 64 )]
		...
		for image_id in image_ids:
			...
			for obj in objects:
				...
				if class_name in self._name_to_class:
					...
					id = self._class_to_ind[self._name_to_class[class_name]]
					boxes=([x1, y1, x2, y2,id])
					res = np.vstack((res,boxes))  # [xmin, ymin, xmax, ymax, label_ind]
...
class ImagenetDataset(data.Dataset):
	...
	def __init__(self, data, root, preproc=None, is_val=False):
		"""Dataset for VID data.
		Args:
		...
		self.is_val = is_val
		if is_val:
			image_sets_file = "module/dataset/val_VID_list.txt"
		else:
			image_sets_file = "module/dataset/train_VID_list.txt"
	...
	def gt_roidb(self):
		...
		if os.path.exists(cache_file):
			print('==>load data from old pkl from {}'.format(cache_file))
			with open(cache_file, 'rb') as fid:
				roidb = pickle.load(fid)
			logging.info('gt roidb loaded from {}'.format(cache_file))
			return roidb
		...
		#gt_roidb = [self.load_vid_annotation(index) for index in range(0, 100)]
		with open(cache_file, 'wb') as fid:
			pickle.dump(gt_roidb, fid, pickle.HIGHEST_PROTOCOL)
		logging.info('wrote gt roidb to {}'.format(cache_file))
	...
	def load_vid_annotation(self, i):
		...
		for obj in objects:
			...
			if class_name in self._name_to_class:
				...
				x2 = float(bbox.find('xmax').text) - 1
				y2 = float(bbox.find('ymax').text) - 1
				bndbox =[x1, y1, x2, y2,self._class_to_ind[self._name_to_class[class_name]]]
